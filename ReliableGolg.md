...
①使用top -Hp <pid>来查看进程中所有线程的 CPU 消耗情况
②通过 jstack <pid> 的输出查看各个线程栈
...
②每个Thread内部都有一个ThreadLocal.ThreadLocalMap类型的成员变量，该成员变量用来存储实际的ThreadLocal变量副本。（ThreadLocalMap的key是ThreadLocal实例本身，value是真正需要存储的Object。）
